...
expression_sizeof_type::create(const location &locn,
    const type::pointer &a_subject, translator_littoral *cntxt)
{
...
expression_sizeof_type::traversal(int)
    const
{
    littoral.print_token("sizeof");
    littoral.print_token("(");
    littoral.print_token(subject->get_name());
    littoral.print_token(")");
}
...
expression_sizeof_type::optimize(void)
    const
{
    return create(get_location(), subject, &littoral);
}
...
expression_sizeof_type::clone(const location &locn)
    const
{
    return create(locn, subject, &littoral);
}
