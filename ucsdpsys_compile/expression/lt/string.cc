...
expression_lt_string::~expression_lt_string()
{
}
...
expression_lt_string::pointer
expression_lt_string::create(const expression::pointer &a_lhs,
    const expression::pointer &a_rhs, translator_compile *cntxt)
{
    return pointer(new expression_lt_string(a_lhs, a_rhs, cntxt));
}


void
expression_lt_string::post_order_traversal(int)
    const
{
...
expression::pointer
expression_lt_string::optimize(void)
    const
{
    ...
    return expression_lt_string::create(e1, e2, &code);
}


expression::pointer
expression_lt_string::clone(const location &locn)
    const
{
...
void
expression_lt_string::logical_traversal(int pprec,
    ...
    bool fall_through_preference) const
{
    ...
    else
    {
        ...
        expression::pointer nep =
            expression_ge_string::create(get_lhs(), get_rhs(), &code);
        nep->traversal(pprec);
