...
expression_lt_real::~expression_lt_real()
{
}
...
expression_lt_real::pointer
expression_lt_real::create(const expression::pointer &a_lhs,
    const expression::pointer &a_rhs, translator_compile *cntxt)
{
    return pointer(new expression_lt_real(a_lhs, a_rhs, cntxt));
}


void
expression_lt_real::post_order_traversal(int)
    const
{
...
expression::pointer
expression_lt_real::optimize(void)
    const
{
    ...
    return expression_lt_real::create(e1, e2, &code);
}


expression::pointer
expression_lt_real::clone(const location &locn)
    const
{
...
void
expression_lt_real::logical_traversal(int pprec,
    ...
    bool fall_through_preference) const
{
    ...
    else
    {
        ...
        expression::pointer nep =
            expression_ge_real::create(get_lhs(), get_rhs(), &code);
        nep->traversal(pprec);
