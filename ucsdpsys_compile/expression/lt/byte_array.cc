...
expression_lt_byte_array::~expression_lt_byte_array()
{
}
...
expression_lt_byte_array::pointer
expression_lt_byte_array::create(const expression::pointer &a_lhs,
    const expression::pointer &a_rhs, translator_compile *a_code)
{
    return pointer(new expression_lt_byte_array(a_lhs, a_rhs, a_code));
}


void
expression_lt_byte_array::traversal(int pprec)
    const
{
...
expression::pointer
expression_lt_byte_array::optimize()
    const
{
    expression::pointer e1 = get_lhs()->optimize();
    expression::pointer e2 = get_rhs()->optimize();

    return expression_lt_byte_array::create(e1, e2, &code);
}


expression::pointer
expression_lt_byte_array::clone(const location &locn)
    const
{
...
void
expression_lt_byte_array::logical_traversal(int pprec,
    ...
    bool fall_through_preference) const
{
    ...
    else
    {
        expression::pointer e2 =
            expression_ge_byte_array::create(get_lhs(), get_rhs(), &code);
        e2->traversal(pprec);
