...
expression_lt_integer::~expression_lt_integer()
{
}
...
expression_lt_integer::pointer
expression_lt_integer::create(const expression::pointer &a_lhs,
    const expression::pointer &a_rhs, translator_compile *cntxt)
{
    return pointer(new expression_lt_integer(a_lhs, a_rhs, cntxt));
}


void
expression_lt_integer::post_order_traversal(int)
    const
{
...
expression::pointer
expression_lt_integer::optimize(void)
    const
{
    ...
    return expression_lt_integer::create(e1, e2, &code);
}


expression::pointer
expression_lt_integer::clone(const location &locn)
    const
{
...
void
expression_lt_integer::logical_traversal(int pprec,
    ...
    bool fall_through_preference) const
{
    ...
    else
    {
        ...
        expression::pointer nep =
            expression_ge_integer::create(get_lhs(), get_rhs(), &code);
        nep->traversal(pprec);
