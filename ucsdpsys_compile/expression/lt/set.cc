...
expression_lt_set::~expression_lt_set()
{
...
expression::pointer
expression_lt_set::create(const expression::pointer &a_lhs,
    const expression::pointer &a_rhs, translator_compile *cntxt)
{
    ...
    {
        expression::pointer e1 = expression_le_set::create(a_lhs, a_rhs, cntxt);
    ...
    pointer result(new expression_lt_set(lhs2, rhs2, cntxt));
    DEBUG(1, "}");
    return result;
}
...
void
expression_lt_set::post_order_traversal(int)
    const
{
    DEBUG(1, "%s", __PRETTY_FUNCTION__);
...
expression::pointer
expression_lt_set::optimize(void)
    const
{
    DEBUG(1, "%s {", __PRETTY_FUNCTION__);
    ...
    expression::pointer result = expression_lt_set::create(e1, e2, &code);
    DEBUG(1, "}");
    return result;
}
...
expression::pointer
expression_lt_set::clone(const location &locn)
    const
{
...
void
expression_lt_set::logical_traversal(int pprec,
