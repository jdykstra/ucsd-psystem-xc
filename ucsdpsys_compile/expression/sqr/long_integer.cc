...
expression_sqr_long_integer::~expression_sqr_long_integer()
{
}
...
expression::pointer
expression_sqr_long_integer::create(const expression::pointer &ep,
    translator_compile *a_code)
{
    if (!a_code->feature_long_integer_extensions())
    {
        ...
        (
            ep->get_location(),
            "the SQR function is not supported for %s",
    ...
    return pointer(new expression_sqr_long_integer(a_operand, a_code));
}


void
expression_sqr_long_integer::post_order_traversal(int)
    const
{
    code.breakpoint(get_location());
    code.emit_ldci(DECOPS_SQR);
    code.emit_byte(CXP);
...
expression::pointer
expression_sqr_long_integer::optimize(void)
    const
{
...
expression::pointer
expression_sqr_long_integer::clone(const location &locn)
    const
{
