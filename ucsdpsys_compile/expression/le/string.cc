...
{
    return pointer(new expression_le_string(a_lhs, a_rhs, cntxt));
}


void
expression_le_string::post_order_traversal(int)
    const
{
    ...
    const
{
    ...
    return expression_le_string::create(e1, e2, &code);
}
...
void
expression_le_string::logical_traversal(int pprec,
    ...
    bool fall_through_preference) const
{
    ...
    else
    {
        ...
        expression::pointer nep =
            expression_gt_string::create(get_lhs(), get_rhs(), &code);
        nep->traversal(pprec);
