...
expression_le_long_integer::~expression_le_long_integer()
{
}
...
expression_le_long_integer::pointer
expression_le_long_integer::create(const expression::pointer &a_lhs,
    const expression::pointer &a_rhs, translator_compile *a_code)
{
    ...
    return pointer(new expression_le_long_integer(e1, e2, a_code));
}


void
expression_le_long_integer::post_order_traversal(int)
    const
{
    code.breakpoint(get_location());
    code.emit_ldci(DECOPS_COMPARE_LE);
...
expression::pointer
expression_le_long_integer::optimize(void)
    const
{
    ...
    return expression_le_long_integer::create(e1, e2, &code);
}


expression::pointer
expression_le_long_integer::clone(const location &locn)
    const
{
...
void
expression_le_long_integer::logical_traversal(int pprec,
    ...
    bool fall_through_preference) const
{
    ...
    else
    {
        ...
        expression::pointer nep =
            expression_gt_long_integer::create(get_lhs(), get_rhs(), &code);
        nep->traversal(pprec);
