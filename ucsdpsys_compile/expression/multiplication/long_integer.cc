...
expression_multiplication_long_integer::expression_multiplication_long_integer(
...
expression_multiplication_long_integer::pointer
expression_multiplication_long_integer::create(const expression::pointer &a_lhs,
    const expression::pointer &a_rhs, translator_compile *a_code)
{
    ...
    return pointer(new expression_multiplication_long_integer(e1, e2, a_code));
}



void
expression_multiplication_long_integer::post_order_traversal(int)
    const
{
    code.breakpoint(get_location());
    code.emit_ldci(DECOPS_MULTIPLY);
    code.emit_byte(CXP);
...
expression::pointer
expression_multiplication_long_integer::optimize(void)
    const
{
...
expression::pointer
expression_multiplication_long_integer::clone(const location &locn)
    const
{
