...
expression_ne_multi_word::~expression_ne_multi_word()
{
}
...
expression_ne_multi_word::pointer
expression_ne_multi_word::create(const expression::pointer &a_lhs,
    const expression::pointer &a_rhs, translator_compile *a_code)
{
    return pointer(new expression_ne_multi_word(a_lhs, a_rhs, a_code));
}


void
expression_ne_multi_word::traversal(int pprec)
    const
{
...
expression::pointer
expression_ne_multi_word::optimize(void)
    const
{
    expression::pointer e1 = get_lhs()->optimize();
    expression::pointer e2 = get_rhs()->optimize();
    return expression_ne_multi_word::create(e1, e2, &code);
}


expression::pointer
expression_ne_multi_word::clone(const location &locn)
    const
{
...
void
expression_ne_multi_word::logical_traversal(int pprec,
    ...
    bool fall_through_preference) const
{
    ...
    else
    {
        expression::pointer e2 =
            expression_eq_multi_word::create(get_lhs(), get_rhs(), &code);
        e2->traversal(pprec);
