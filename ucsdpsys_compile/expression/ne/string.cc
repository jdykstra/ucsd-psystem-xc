...
expression_ne_string::~expression_ne_string()
{
}
...
expression_ne_string::pointer
expression_ne_string::create(const expression::pointer &a_lhs,
    const expression::pointer &a_rhs, translator_compile *cntxt)
{
    return pointer(new expression_ne_string(a_lhs, a_rhs, cntxt));
}


void
expression_ne_string::post_order_traversal(int)
    const
{
...
expression::pointer
expression_ne_string::optimize(void)
    const
{
    ...
    return expression_ne_string::create(e1, e2, &code);
}


expression::pointer
expression_ne_string::clone(const location &locn)
    const
{
...
void
expression_ne_string::logical_traversal(int pprec,
