...
expression_ne_real::~expression_ne_real()
{
}
...
expression_ne_real::pointer
expression_ne_real::create(const expression::pointer &a_lhs,
    const expression::pointer &a_rhs, translator_compile *cntxt)
{
    return pointer(new expression_ne_real(a_lhs, a_rhs, cntxt));
}


void
expression_ne_real::post_order_traversal(int)
    const
{
...
expression::pointer
expression_ne_real::optimize(void)
    const
{
    ...
    return expression_ne_real::create(e1, e2, &code);
}


expression::pointer
expression_ne_real::clone(const location &locn)
    const
{
...
void
expression_ne_real::logical_traversal(int pprec,
