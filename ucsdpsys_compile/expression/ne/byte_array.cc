...
expression_ne_byte_array::~expression_ne_byte_array()
{
}
...
expression_ne_byte_array::pointer
expression_ne_byte_array::create(const expression::pointer &a_lhs,
    const expression::pointer &a_rhs, translator_compile *a_code)
{
    return pointer(new expression_ne_byte_array(a_lhs, a_rhs, a_code));
}


void
expression_ne_byte_array::traversal(int pprec)
    const
{
...
expression::pointer
expression_ne_byte_array::optimize(void)
    const
{
    ...
    return expression_ne_byte_array::create(e1, e2, &code);
}


expression::pointer
expression_ne_byte_array::clone(const location &locn)
    const
{
...
void
expression_ne_byte_array::logical_traversal(int pprec,
    ...
    bool fall_through_preference) const
{
    ...
    else
    {
        expression::pointer e2 =
            expression_eq_byte_array::create(get_lhs(), get_rhs(), &code);
        e2->traversal(pprec);
