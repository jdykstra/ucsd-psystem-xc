...
expression_addition_long_integer::~expression_addition_long_integer()
{
}
...
expression_addition_long_integer::expression_addition_long_integer(
    ...
    code(*a_code)
{
    assert(type_long_integer_sized::is_a(a_lhs));
    assert(type_long_integer_sized::is_a(a_rhs));
    code.need_segment(DECOPS_SEG_NUM);
}


expression_addition_long_integer::pointer
expression_addition_long_integer::create(const expression::pointer &a_lhs,
    const expression::pointer &a_rhs, translator_compile *a_code)
{
    ...
    return pointer(new expression_addition_long_integer(e1, e2, a_code));
}


void
expression_addition_long_integer::post_order_traversal(int)
    const
{
    code.breakpoint(get_location());
    code.emit_ldci(DECOPS_ADD);
    code.emit_byte(CXP);
...
expression::pointer
expression_addition_long_integer::optimize(void)
    const
{
...
expression::pointer
expression_addition_long_integer::clone(const location &locn)
    const
{
