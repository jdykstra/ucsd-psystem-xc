...
expression_writeln_string::~expression_writeln_string()
{
}
...
expression_writeln_string::pointer
expression_writeln_string::create(const expression::pointer &a_value,
    const expression::pointer &a_width, translator_compile *a_context)
{
    return pointer(new expression_writeln_string(a_value, a_width, a_context));
}


expression::pointer
expression_writeln_string::function_call_expression_factory(
    const expression::pointer &file) const
{
    expression_list args;
    expression::pointer ep =
        context->name_expression(file->get_location(), "sys:fwritestr");
    args.push_back(ep);
    args.push_back(file);
...
rcstring
expression_writeln_string::lisp_representation(void)
    const
{
    return
        (
...
expression::pointer
expression_writeln_string::optimize(void)
    const
{
...
expression::pointer
expression_writeln_string::clone(const location &locn)
    const
{
