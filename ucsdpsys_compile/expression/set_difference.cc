...
expression_set_difference::~expression_set_difference()
{
}
    ...
    rhs(a_rhs)
{
    ...
    if (t1 != t2)
    {
        ...
        (
            get_location(),
            "invalid set difference, given a %s and a %s",
...
expression_set_difference::pointer
expression_set_difference::create(const expression::pointer &a_lhs,
    const expression::pointer &a_rhs, translator_compile *cntxt)
{
    ...
    return pointer(new expression_set_difference(lhs2, rhs2, cntxt));
}


void
expression_set_difference::traversal(int)
    const
{
    lhs->traversal(0);
    rhs->traversal(0);
...
expression::pointer
expression_set_difference::optimize(void)
    const
{
    DEBUG(1, "%s", __PRETTY_FUNCTION__);
    expression::pointer lop = lhs->optimize();
    DEBUG(2, "lop = %s", lop->lisp_representation().c_str());
    assert(type_set_sized::is_a(lop->get_type()));
    expression::pointer rop = rhs->optimize();
    DEBUG(2, "rop = %s", rop->lisp_representation().c_str());
    ...
    if (lop->is_constant() && rop->is_constant())
    {
        ...
        DEBUG(2, "s = %s", s.representation().c_str());
    ...
    DEBUG(2, "not much better");
    return expression_set_difference::create(lop, rop, &code);
}


expression::pointer
expression_set_difference::clone(const location &locn)
    const
{
...
type::pointer
expression_set_difference::get_type()
    const
{
    return lhs->get_type();
}
...
void
expression_set_difference::logical_traversal(int, label::pointer &,
    label::pointer &, bool) const
{
    assert(!"set diff is not bool");
}


int
expression_set_difference::get_precedence()
    const
{
    return prec_add;
}
...
bool
expression_set_difference::is_lvalue()
    const
{
    return false;
}
...
expression_set_difference::has_side_effect()
    const
{
...
rcstring
expression_set_difference::lisp_representation(void)
    const
{
    return
        (
