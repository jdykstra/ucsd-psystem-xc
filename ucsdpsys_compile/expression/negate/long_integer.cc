...
expression_negate_long_integer::~expression_negate_long_integer()
{
}
...
expression_negate_long_integer::pointer
expression_negate_long_integer::create(const expression::pointer &a_operand,
    translator_compile *a_code)
{
    type::pointer tp = type_long_integer_sized::create();
    expression::pointer ep =
        expression_lint2lint_compile::create(tp, a_operand, a_code);
    return pointer(new expression_negate_long_integer(ep, a_code));
}


void
expression_negate_long_integer::post_order_traversal(int)
    const
{
    code.breakpoint(get_location());
    code.emit_ldci(DECOPS_NEGATE);
    code.emit_byte(CXP);
...
expression::pointer
expression_negate_long_integer::optimize(void)
    const
{
...
expression::pointer
expression_negate_long_integer::clone(const location &locn)
    const
{
