...
expression_gt_integer::~expression_gt_integer()
{
}
...
expression_gt_integer::pointer
expression_gt_integer::create(const expression::pointer &a_lhs,
    const expression::pointer &a_rhs, translator_compile *cntxt)
{
    return pointer(new expression_gt_integer(a_lhs, a_rhs, cntxt));
}


void
expression_gt_integer::post_order_traversal(int)
    const
{
...
expression::pointer
expression_gt_integer::optimize(void)
    const
{
    ...
    return expression_gt_integer::create(e1, e2, &code);
}


expression::pointer
expression_gt_integer::clone(const location &locn)
    const
{
...
void
expression_gt_integer::logical_traversal(int pprec,
    ...
    bool fall_through_preference) const
{
    ...
    else
    {
        ...
        expression::pointer nep =
            expression_le_integer::create(get_lhs(), get_rhs(), &code);
        nep->traversal(pprec);
