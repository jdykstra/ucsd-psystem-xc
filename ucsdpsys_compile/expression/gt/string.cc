...
expression_gt_string::~expression_gt_string()
{
}
...
expression_gt_string::pointer
expression_gt_string::create(const expression::pointer &a_lhs,
    const expression::pointer &a_rhs, translator_compile *cntxt)
{
    return pointer(new expression_gt_string(a_lhs, a_rhs, cntxt));
}


void
expression_gt_string::post_order_traversal(int)
    const
{
...
expression::pointer
expression_gt_string::optimize(void)
    const
{
    ...
    return expression_gt_string::create(e1, e2, &code);
}


expression::pointer
expression_gt_string::clone(const location &locn)
    const
{
...
void
expression_gt_string::logical_traversal(int pprec,
    ...
    bool fall_through_preference) const
{
    ...
    else
    {
        ...
        expression::pointer nep =
            expression_le_string::create(get_lhs(), get_rhs(), &code);
        nep->traversal(pprec);
