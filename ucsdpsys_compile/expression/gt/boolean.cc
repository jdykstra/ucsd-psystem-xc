...
expression_gt_boolean::~expression_gt_boolean()
{
}
...
expression_gt_boolean::pointer
expression_gt_boolean::create(const expression::pointer &a_lhs,
    const expression::pointer &a_rhs, translator_compile *cntxt)
{
    return pointer(new expression_gt_boolean(a_lhs, a_rhs, cntxt));
}


void
expression_gt_boolean::post_order_traversal(int)
    const
{
...
expression::pointer
expression_gt_boolean::optimize(void)
    const
{
    ...
    return expression_gt_boolean::create(e1, e2, &code);
}


expression::pointer
expression_gt_boolean::clone(const location &locn)
    const
{
...
void
expression_gt_boolean::logical_traversal(int pprec,
    ...
    bool fall_through_preference) const
{
    ...
    else
    {
        ...
        expression::pointer nep =
            expression_le_boolean::create(get_lhs(), get_rhs(), &code);
        nep->traversal(pprec);
