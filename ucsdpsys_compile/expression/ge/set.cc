...
expression_ge_set::~expression_ge_set()
{
...
expression_ge_set::pointer
expression_ge_set::create(const expression::pointer &a_lhs,
    const expression::pointer &a_rhs, translator_compile *cntxt)
{
    ...
    pointer result(new expression_ge_set(lhs2, rhs2, cntxt));
    DEBUG(1, "}");
    return result;
}
...
void
expression_ge_set::post_order_traversal(int)
    const
{
    DEBUG(1, "%s", __PRETTY_FUNCTION__);
...
expression::pointer
expression_ge_set::optimize(void)
    const
{
    DEBUG(1, "%s {", __PRETTY_FUNCTION__);
    ...
    expression::pointer result = expression_ge_set::create(e1, e2, &code);
    DEBUG(1, "}");
    return result;
}
...
expression::pointer
expression_ge_set::clone(const location &locn)
    const
{
...
void
expression_ge_set::logical_traversal(int pprec,
