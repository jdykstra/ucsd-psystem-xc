...
{
    return pointer(new expression_ge_boolean(a_lhs, a_rhs, cntxt));
}


void
expression_ge_boolean::post_order_traversal(int)
    const
{
    ...
    const
{
    ...
    return expression_ge_boolean::create(e1, e2, &code);
}
...
void
expression_ge_boolean::logical_traversal(int pprec,
    ...
    bool fall_through_preference) const
{
    ...
    else
    {
        ...
        expression::pointer nep =
            expression_lt_boolean::create(get_lhs(), get_rhs(), &code);
        nep->traversal(pprec);
