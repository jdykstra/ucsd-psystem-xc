...
expression_ge_real::~expression_ge_real()
{
}
...
expression_ge_real::pointer
expression_ge_real::create(const expression::pointer &a_lhs,
    const expression::pointer &a_rhs, translator_compile *cntxt)
{
    return pointer(new expression_ge_real(a_lhs, a_rhs, cntxt));
}


void
expression_ge_real::post_order_traversal(int)
    const
{
...
expression::pointer
expression_ge_real::optimize(void)
    const
{
    ...
    return expression_ge_real::create(e1, e2, &code);
}


expression::pointer
expression_ge_real::clone(const location &locn)
    const
{
...
void
expression_ge_real::logical_traversal(int pprec,
    ...
    bool fall_through_preference) const
{
    ...
    else
    {
        ...
        expression::pointer nep =
            expression_lt_real::create(get_lhs(), get_rhs(), &code);
        nep->traversal(pprec);
