...
{
    return pointer(new expression_ge_string(a_lhs, a_rhs, cntxt));
}


void
expression_ge_string::post_order_traversal(int)
    const
{
    ...
    const
{
    ...
    return expression_ge_string::create(e1, e2, &code);
}
...
void
expression_ge_string::logical_traversal(int pprec,
    ...
    bool fall_through_preference) const
{
    ...
    else
    {
        ...
        expression::pointer nep =
            expression_lt_string::create(get_lhs(), get_rhs(), &code);
        nep->traversal(pprec);
