...
expression_eq_real::~expression_eq_real()
{
}
...
expression_eq_real::pointer
expression_eq_real::create(const expression::pointer &a_lhs,
    const expression::pointer &a_rhs, translator_compile *cntxt)
{
    return pointer(new expression_eq_real(a_lhs, a_rhs, cntxt));
}


void
expression_eq_real::post_order_traversal(int)
    const
{
    ...
    const
{
    ...
    return expression_eq_real::create(e1, e2, &code);
}


expression::pointer
expression_eq_real::clone(const location &locn)
    const
{
...
void
expression_eq_real::logical_traversal(int pprec,
    ...
    bool fall_through_preference) const
{
    ...
    else
    {
        ...
        expression::pointer nep =
            expression_ne_real::create(get_lhs(), get_rhs(), &code);
        nep->traversal(pprec);
