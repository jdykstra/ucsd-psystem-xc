...
expression_eq_set::~expression_eq_set()
{
...
expression_eq_set::pointer
expression_eq_set::create(const expression::pointer &a_lhs,
    const expression::pointer &a_rhs, translator_compile *cntxt)
{
    ...
    pointer result(new expression_eq_set(lhs2, rhs2, cntxt));
    DEBUG(1, "}");
    return result;
}
...
void
expression_eq_set::post_order_traversal(int)
    const
{
    DEBUG(1, "%s", __PRETTY_FUNCTION__);
...
expression::pointer
expression_eq_set::optimize(void)
    const
{
    DEBUG(1, "%s {", __PRETTY_FUNCTION__);
    ...
    expression::pointer result = expression_eq_set::create(e1, e2, &code);
    DEBUG(1, "}");
    return result;
}
...
expression::pointer
expression_eq_set::clone(const location &locn)
    const
{
...
void
expression_eq_set::logical_traversal(int pprec,
    ...
    bool fall_through_preference) const
{
    ...
    else
    {
        ...
        expression::pointer nep =
            expression_ne_set::create(get_lhs(), get_rhs(), &code);
        nep->traversal(pprec);
