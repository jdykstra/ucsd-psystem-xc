...
#include <ucsdpsys_compile/symbol/variable/normal/globref.h>


symbol_variable_normal_globref::~symbol_variable_normal_globref()
{
...
symbol_variable_normal_globref::pointer
symbol_variable_normal_globref::create(const variable_name &a_name,
    const type::pointer &a_tp, translator_compile *a_context)
{
    return pointer(new symbol_variable_normal_globref(a_name, a_tp, a_context));
}


expression::pointer
symbol_variable_normal_globref::name_expression_factory(
    const symbol::pointer &sp, const location &where, int)
{
    ...
    if (type_reference::is_a(tp))
    {
        ...
        addr =
            ...
            (
                expression_address_globref::create
                (
                    where,
