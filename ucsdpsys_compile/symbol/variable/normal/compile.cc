...
symbol_variable_normal_compile::~symbol_variable_normal_compile()
{
...
symbol_variable_normal_compile::pointer
symbol_variable_normal_compile::create(const variable_name &a_name,
    ...
    translator_compile *a_context)
{
    ...
    return
        pointer
        (
            new symbol_variable_normal_compile
            (
                a_name,
                a_tp,
                a_lex_level,
...
expression::pointer
symbol_variable_normal_compile::name_expression_factory(
    const symbol::pointer &sp, const location &where, int ref_lxlvl)
{
    DEBUG(1, "%s", __PRETTY_FUNCTION__);
    assert(this == sp.get());
    expression::pointer addr;
    type::pointer tp = get_type();
    bit_address offset = get_bit_offset();
    assert(offset.on_word_boundary());
    if (type_reference::is_a(tp))
    {
        ...
        if (get_lex_level() == ref_lxlvl)
        {
            ...
            addr =
                ...
                (
                    ...
                    (
                        where,
                        // The plus one is because the opcodes count 1
                        // based, but our symbol address info is 0 based.
                        offset.get_word_addr() + 1,
        ...
        else
        {
            ...
            assert(ref_lxlvl > get_lex_level());
            addr =
                expression_load_indirect::create
                (
