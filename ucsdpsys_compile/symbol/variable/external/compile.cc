...
symbol_variable_external_compile::~symbol_variable_external_compile()
{
...
symbol_variable_external_compile::symbol_variable_external_compile(
    ...
    symbol_variable_external(a_name, a_tp, a_segnum),
    context(a_context)
{
    DEBUG(1, "%s", __PRETTY_FUNCTION__);
}
...
symbol_variable_external_compile::pointer
symbol_variable_external_compile::create(const variable_name &a_name,
    const type::pointer &a_tp, int a_segnum, translator_compile *a_context)
{
    DEBUG(1, "%s", __PRETTY_FUNCTION__);
    return
        pointer
        (
            new symbol_variable_external_compile
            (
                a_name,
                a_tp,
                a_segnum,
...
expression::pointer
symbol_variable_external_compile::name_expression_factory(
    const symbol::pointer &sym, const location &where, int)
{
    DEBUG(1, "%s", __PRETTY_FUNCTION__);
    assert(this == sym.get());
    expression::pointer addr;
    type::pointer tp = get_type();
    bit_address boffset = get_bit_offset();
    int woffset = boffset.get_word_addr() + 1;
    if (type_reference::is_a(tp))
    {
        ...
        addr =
            ...
            (
                expression_address_external::create
                (
                    where,
