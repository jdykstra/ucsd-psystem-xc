...
symbol_function_cos::~symbol_function_cos()
{
    DEBUG(1, "%s", __PRETTY_FUNCTION__);
}


symbol_function_cos::symbol_function_cos(translator_compile *cntxt) :
    symbol_function("cos", location::builtin(), scope::pointer(), -1),
    code(*cntxt)
{
    //
...
symbol_function_cos::pointer
symbol_function_cos::create(translator_compile *cntxt)
{
    DEBUG(1, "%s", __PRETTY_FUNCTION__);
    return pointer(new symbol_function_cos(cntxt));
}


expression::pointer
symbol_function_cos::function_call_expression_factory(
    const expression_list &rhs) const
{
    ...
    //
    return expression_function_call_cos::create(rhs2.tail(), &code);
}


expression::pointer
symbol_function_cos::name_expression_factory(const symbol::pointer &sym,
