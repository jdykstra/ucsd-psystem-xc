...
#include <ucsdpsys_compile/expression/function_call/moveleft.h>
#include <ucsdpsys_compile/expression/integer/compile.h>
#include <ucsdpsys_compile/symbol/function/moveleft.h>
#include <ucsdpsys_compile/translator/compile.h>


symbol_function_moveleft::~symbol_function_moveleft()
{
}


symbol_function_moveleft::symbol_function_moveleft(translator_compile *cntxt) :
    symbol_function("moveleft", location::builtin(), scope::pointer(), -1),
    code(*cntxt)
{
    //
    // procedure moveleft(var src; var dest; count: integer);
    //
    name_type_list args;
    {
...
symbol_function_moveleft::pointer
symbol_function_moveleft::create(translator_compile *cntxt)
{
    return pointer(new symbol_function_moveleft(cntxt));
}


expression::pointer
symbol_function_moveleft::function_call_expression_factory(
    const expression_list &rhs) const
{
    ...
    // or, it could be a[n]
    {
        ...
        if (!ep)
        {
            ...
            (
                a1->get_location(),
                "function %s: parameter one: must be a packed array of char, "
                    "not a %s",
                get_name().c_str(),
                a1->get_description().c_str()
            );
            DEBUG(1, "}");
            return expression_error::create(a1->get_location());
        }
        args.push_back(ep);
    }
...

    {
        ...
        if (ep->is_constant())
        {
            ...
            if (value <= 0)
            {
                ...
                // statement" processing will do that.
    ...
    //
    return expression_function_call_moveleft::create(args, &code);
}


expression::pointer
symbol_function_moveleft::name_expression_factory(const symbol::pointer &sym,
