...
symbol_function_exp::~symbol_function_exp()
{
    DEBUG(1, "%s", __PRETTY_FUNCTION__);
}


symbol_function_exp::symbol_function_exp(translator_compile *cntxt) :
    symbol_function("exp", location::builtin(), scope::pointer(), -1),
    code(*cntxt)
{
    //
...
symbol_function_exp::pointer
symbol_function_exp::create(translator_compile *cntxt)
{
    DEBUG(1, "%s", __PRETTY_FUNCTION__);
    return pointer(new symbol_function_exp(cntxt));
}


expression::pointer
symbol_function_exp::function_call_expression_factory(
    const expression_list &rhs) const
{
    ...
    //
    return expression_function_call_exp::create(rhs2.tail(), &code);
}


expression::pointer
symbol_function_exp::name_expression_factory(const symbol::pointer &sym,
