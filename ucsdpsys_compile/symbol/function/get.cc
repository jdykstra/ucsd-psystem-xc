...
symbol_function_get::~symbol_function_get()
{
    DEBUG(1, "%s", __PRETTY_FUNCTION__);
}


symbol_function_get::symbol_function_get(translator_compile *cntxt) :
    symbol_function("get", location::builtin(), scope::pointer(), -1),
    code(*cntxt)
{
    //
    // procedure get(var f: file of anything);
    //
    DEBUG(1, "%s", __PRETTY_FUNCTION__);
    name_type_list args;
    {
...
symbol_function_get::pointer
symbol_function_get::create(translator_compile *cntxt)
{
    DEBUG(1, "%s", __PRETTY_FUNCTION__);
    return pointer(new symbol_function_get(cntxt));
}


expression::pointer
symbol_function_get::function_call_expression_factory(
    const expression_list &rhs) const
{
    ...
    expression::pointer func =
        code.name_expression(rhs.get_location(), "sys:fget");
    args.push_back(func);
...
expression::pointer
symbol_function_get::name_expression_factory(const symbol::pointer &sym,
