...
TEST_SUBJECT="assembler listing"
. test_prelude
...
cat > listing.ok << 'fubar'
Assembler Listing: example.text                                           Page 1
...
; ----- -----  --------  -------- --- --------- --------
;     1    36  EXAMPLE   SEPRTSEG   1 6502      3.0
;              EXAMPLE   SEPPROC          1         0
...
ucsdpsys_assemble -Llisting.txt example.text
test $? -eq 0 || fail

diff -u listing.ok listing.txt
test $? -eq 0 || fail

ucsdpsys_disassemble -a example.code -o test.out
test $? -eq 0 || no_result

diff code.ok test.out
test $? -eq 0 || fail
