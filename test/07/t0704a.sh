...
TEST_SUBJECT="assembler vs include"
. test_prelude
...
cat > example.text << 'fubar'
     .arch "6502"
     .include incl.text;comment
     .end
fubar
test $? -eq 0 || no_result

cat > xxx/yyy/incl.text << 'fubar'
     .proc example
     rts
fubar
test $? -eq 0 || no_result
...
; ----- -----  --------  -------- --- --------- --------
;     1    18  EXAMPLE   SEPRTSEG   1 6502      3.0
;              EXAMPLE   SEPPROC          1         0
...
ucsdpsys_assemble -Jxxx -Iyyy example.text
test $? -eq 0 || fail
