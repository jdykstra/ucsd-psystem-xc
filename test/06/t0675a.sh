...
TEST_SUBJECT="ucsdpsys_osmakgen vs disk size"
. test_prelude
...
filenames = etc/host-disk-kb.sh example/main.text

stage1/$(host)/codefiles/example/main.code: example/main.text
{TAB}@mkdir -p stage1/$(host)/codefiles/example
{TAB}ucsdpsys_compile $(XCFLAGS) -Iexample -o \
{TAB}{TAB}stage1/$(host)/codefiles/example/main.code example/main.text
...
{TAB}cp stage1/$(host)/codefiles/example/main.code \
{TAB}{TAB}stage1/$(host)/utility/example.code

# build stage one utility disk image

stage1/$(host)/utility.vol: stage1/$(host)/utility/example.code
{TAB}@mkdir -p stage1/$(host)
{TAB}ucsdpsys_mkfs --arch=$(arch) -B$(host_disk_kb) --label=utility \
...
{TAB}cp stage2/$(host)/codefiles/example/main.code \
{TAB}{TAB}stage2/$(host)/utility/example.code
...
{TAB}cp stage3/$(host)/codefiles/example/main.code \
{TAB}{TAB}stage3/$(host)/utility/example.code
...
clean: clean-files
clean-files:
{TAB}rm -f stage1/$(host)/codefiles/example/main.code
{TAB}rm -f stage1/$(host)/utility.vol stage1/$(host)/utility/example.code
{TAB}rm -f stage2/$(host)/utility.vol stage2/$(host)/utility/example.code
{TAB}rm -f stage3/$(host)/utility.vol stage3/$(host)/utility/example.code
...
touch example/main.text
test $? -eq 0 || no_result

ucsdpsys_osmakgen -o test.makefile.out -b example/main.text etc/host-disk-kb.sh
test $? -eq 0 || fail
