...
opcode_pseudo_host::~opcode_pseudo_host()
{
}


opcode_pseudo_host::opcode_pseudo_host(assembler *a_context) :
    opcode_pseudo(a_context)
{
}


opcode_pseudo_host::pointer
opcode_pseudo_host::create(assembler *a_context)
{
    return pointer(new opcode_pseudo_host(a_context));
}


void
opcode_pseudo_host::interpret(const expression_list &args)
{
...
rcstring
opcode_pseudo_host::get_name(void)
    const
{
    return ".host";
}
...
opcode::kind_t
opcode_pseudo_host::get_kind(void)
    const
{
    return kind_prelude;
}
...
bool
opcode_pseudo_host::next_argument_always_a_string(void)
    const
{
    return true;
}
